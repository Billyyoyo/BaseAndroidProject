...
public class WriteNoteFragment extends BaseFragment implements WriteFragment {
    ...
    public int getLayoutId() {
        return R.layout.fragment_write_note;
    }

    public Note doPublish() {
        Note note = new Note();
        note.title = titleBox.getText().toString();
        note.content = contentBox.getText().toString();
        note.icon = Util.joinString(imgPaths, ",");
        note.type = Note.TYPE_NORMAL;
            ...
            if (requestCode == PhotoPicker.REQUEST_CODE) {//拍照或者选择照片
                if (data != null) {
                    ...
                    if (photos != null && !photos.isEmpty()) {
                        addBtn.setVisibility(imgPaths.size() >= 9 ? View.GONE : View.VISIBLE);
    ...
    private void addPhotoView(final String path) {
        imgPaths.add(path);
        ViewGroup.LayoutParams lp = new ViewGroup.LayoutParams(Util.dip2px(getContext(), 100), Util.dip2px(getContext(), 100));
